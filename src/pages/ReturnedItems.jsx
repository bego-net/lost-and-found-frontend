...
import { Search, MapPin, Filter, CheckCircle2, X, Calendar, Tag, ArrowUpDown, Clock } from "lucide-react";
...
function ReturnedItems() {
  ...
  useEffect(() => {
    async function fetchReturned() {
      try {
        setLoading(true);
        const res = await api.get("/items?status=returned");
      ...
      <div className="relative flex flex-col items-center justify-center pt-6 pb-12 px-6 overflow-hidden">
        ...
        <div className="relative z-10 text-center space-y-6 max-w-3xl">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-[10px] font-black uppercase tracking-[0.25em]">
            <CheckCircle2 size={14} /> Reunited belongings
          </div>
          
          <h1 className="text-5xl md:text-7xl font-black text-slate-900 dark:text-white tracking-tighter">
            Returned <span className="text-emerald-600">Items</span>
          </h1>
          
          <p className="text-slate-500 dark:text-slate-400 font-medium text-base md:text-lg">
            A wall of success stories! Items that have been successfully returned to their rightful owners.
          </p>
          ...
          <div className="flex items-center w-full max-w-xl mx-auto p-1.5 bg-white dark:bg-slate-900 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.06)] border border-slate-200 dark:border-slate-800">
            ...
            <button 
              ...
              className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${
                showFilters ? "bg-emerald-600 text-white" : "bg-slate-100 dark:bg-slate-800 text-slate-600"
              }`}
            >
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-slate-50 dark:bg-slate-900/20 rounded-[3rem] border-2 border-dashed border-slate-200 dark:border-slate-800">
            ...
            <h3 className="text-lg font-black text-slate-900 dark:text-white">No items returned yet</h3>
            <button onClick={() => {setFilters({category: 'all', location: '', sortOrder: 'newest'}); setSearch("")}} className="mt-4 text-[10px] font-black uppercase text-emerald-600 underline underline-offset-4">Reset all</button>
          </div>
        ) : (
          <div className="grid gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {filtered.map((item) => (
